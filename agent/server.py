...
from agent.graph import build_graph, load_local_tools, load_youtube_tools, load_sefaria_tools
...
async def lifespan(app: FastAPI):
    ...
    local_tools, local_client = await load_local_tools()
    youtube_tools, youtube_client = await load_youtube_tools()
    sefaria_tools, sefaria_client = await load_sefaria_tools()
    _mcp_clients = [c for c in [local_client, youtube_client, sefaria_client] if c is not None]
    _graph = await build_graph(extra_tools=local_tools + youtube_tools + sefaria_tools)
    yield
